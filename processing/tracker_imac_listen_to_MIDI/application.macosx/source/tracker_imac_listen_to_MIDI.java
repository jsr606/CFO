...
public class tracker_imac_listen_to_MIDI extends PApplet {
...
public void tick() {
  ...
  for (int i = 0; i<byteTransmit.length; i++) {
    ...
    if (monitor) print(PApplet.parseInt(byteTransmit[i])+"\t");
  }
  if (monitor) println();
}

public void compileNotes() {
  String notes = "";
  notes += (""+steps)+PApplet.parseChar(9);
  for (int i = 0; i<steps; i++) {
    String stepData = (String) trackData.get(i);
    parseNotes(stepData);

    notes += PApplet.parseInt(byteTransmit[1])+","+PApplet.parseInt(byteTransmit[2]);
    if (i != steps-1) notes += ",";
  }
  //println();
  println(notes);
  cp.copyString(notes+PApplet.parseChar(10));
}

public void compileSequence() {
  String notes = "";
  //notes += (""+steps)+char(9);
  for (int i = 0; i<steps; i++) {
    String stepData = (String) trackData.get(i);
    ...
    notes += PApplet.parseInt(byteTransmit[1])+","+PApplet.parseInt(byteTransmit[2]);
    parseCommand(stepData);    
    notes += ","+PApplet.parseInt(byteTransmit[3])+","+PApplet.parseInt(byteTransmit[4])+",";
    //if (i != steps-1) notes += ",";
  }
  //println();
  println(notes);
...
public void compileCommands() {
  ...
  for (int i = 0; i<steps; i++) {
    ...
    commands += PApplet.parseInt(byteTransmit[3])+","+PApplet.parseInt(byteTransmit[4]);

    if (i != steps-1) commands += ",";
  }
  println(commands);
  cp.copyString(commands+PApplet.parseChar(10));
}

public void parseID(String data) {
  //parse ID
  if (ignoreID) {
    byteTransmit[0] = PApplet.parseByte(128+42); //'*'
    return;
  }
  ...
  if (ID2 == ' ') { //empty
    byteTransmit[0] = PApplet.parseByte(activeID); //255 is the new 32
  } 
  else if (ID2 == '*') {
    byteTransmit[0] = PApplet.parseByte(128+42); //'*'
  } 
    ...
    if (ID == -1) {
      byteTransmit[0] = PApplet.parseByte(255);
      //print("unknown ID");
    } 
    else {
      byteTransmit[0] = PApplet.parseByte(ID);
    }
  }
}

public void parseNotes(String data) {
  //note
  if (data.charAt(xPos[NOTE_POS]) == ' ') {
    //no notes here
    byteTransmit[1] = PApplet.parseByte(255);
    byteTransmit[2] = PApplet.parseByte(255);
  } 
  else {
    ...
    if (noteVal != -1) {
      byteTransmit[1] = PApplet.parseByte(noteVal);

      //add octave
      char oct = data.charAt(xPos[NOTE_POS+2]);
      byte octVal = PApplet.parseByte(oct-48);
      if (octVal >= 0 && octVal <10) {
        byteTransmit[2] = PApplet.parseByte(octVal);
      }
      else {
        //octave field empty
        byteTransmit[2] = PApplet.parseByte(255);
...
public void parseCommand(String data) {
  ...
  if (data.charAt(xPos[COMMAND_POS]) == ' ') {
    //no commands here
    byteTransmit[3] = PApplet.parseByte(255);
    byteTransmit[4] = PApplet.parseByte(255);
  } 
  else {
    //parse command + value
    ...
    case 'K':
      //KILL
      byteTransmit[3] = PApplet.parseByte(128+75); //'K'
      int killTime = getInt3(value3,value1,value2); //killTime defines millis() delay to kill command
      byteTransmit[4] = PApplet.parseByte(killTime);
      break;

    case 'W':
      if (command2 == ' ' || command2 == 'A') {
        //absolute
        byteTransmit[3] = PApplet.parseByte(128+87); //'W'
        int value = getInt (value1,value2);
        byteTransmit[4] = PApplet.parseByte(value);
      }
      break;

    case 'B':
      byteTransmit[3] = PApplet.parseByte(128+66); //'B'
      int bpmValue = getInt3(value3,value1,value2);
      //print("BPM: "+bpmValue);
      byteTransmit[4] = PApplet.parseByte(bpmValue);
      bpmTracker = PApplet.parseInt(map(bpmValue,1,255,1,1000));
      calculateDelays();
      break;
    ...
    case 'O':
      ...
      if (command2 == ' ' || command2 == 'C') {
        //absolute
        byteTransmit[3] = PApplet.parseByte(128+79); //'O'
        int value = getInt (value1,value2);
        byteTransmit[4] = PApplet.parseByte(value);
        break;
      }
    ...
    case 'G':
      ...
      if (command2 == '+') {
        //add
        byteTransmit[3] = PApplet.parseByte(128+103); //'g'
        if (value1 == ' ') value1 ='0';
        int value = getInt (value1,value2);
        //println("gain+: "+value);
        byteTransmit[4] = PApplet.parseByte(64+value);
      ...
      byteTransmit[3] = PApplet.parseByte(128+71); //'G'
      if (value1 == ' ') value1 ='0';
      if (value3 == ' ') value3 ='0';
...
public int getInt (char c1, char c2) {
...
public int getInt3 (char c1, char c2, char c3) {   
  if (c1 == 32 && c2 == 32) return -1; 
  if (c1 == 32) c1 = '0';
  if (c2 == 32) c2 = '0';
  char cCollect[] = {
...
public void stop() {
  println("bye");
  serialPort.write(PApplet.parseByte(128+2));
  serialPort.write(PApplet.parseByte(128+42));
  serialPort.write(PApplet.parseByte(255));
  serialPort.write(PApplet.parseByte(255));
  serialPort.write(PApplet.parseByte(128+75)); //KILL command
  serialPort.write(PApplet.parseByte(255));
} 
...
public void controllerChangeReceived(rwmidi.Controller cc) {
  ...
  if (theController >=14 && theController < 20) {
    ...
    int ledBrightness = PApplet.parseInt(map(theControllerValue, 0, 127, 0, 255));
...
public void noteOnReceived(Note note) {
  ...
  if (noteVal >= 48) {
    ...
    byteTransmit[0] = PApplet.parseByte(128+42); //'*' = all synth IDs
    
    print("got midi val: "+noteVal);
    int sendNote = (noteVal-48)%12;
    print("\tthis translates to note "+sendNote);
    byteTransmit[1] = PApplet.parseByte(sendNote);
    int oct = PApplet.parseInt((noteVal-48)/12);
    println(" in octave "+oct);
    byteTransmit[2] = PApplet.parseByte(oct);
    byteTransmit[3] = PApplet.parseByte(128+103); //'g' == gain
    byteTransmit[4] = PApplet.parseByte(velocity);
    ...
    for (int i = 0; i<byteTransmit.length; i++) {
      serialPort.write(byteTransmit[i]);
      print(PApplet.parseInt(byteTransmit[i])+"\t");
    }
    println();
  }
}
...
class ClipHelper {
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "tracker_imac_listen_to_MIDI" });
  }
}
